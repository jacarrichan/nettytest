...
/**
 * 该类的方法基于bigEndian模式<br>
 * 可参见 @see java.nio.Bits
 * 
 * @author jacarri
 * 
 */
public class StreamUtils {
	...
	public void write(byte[] aValue, int aFrom, int aLength) {
		...
		int tmp = 0;
		while (tmp < aLength) {
			buffer[writepos++] = aValue[aFrom + tmp];
			tmp++;
		}
		size += aLength;
	}
	...
	public void replaceShort(int from, short newshort) {
		...
		intbyte[1] = (byte) ((newshort) & 0xFF);
		intbyte[0] = (byte) ((newshort >> 8) & 0xFF);
	...
	public void replace(int from, int length, byte[] data, int dataOffset, int dataLength) {
		...
		if (sizeoffset > 0) {
			...
			System.arraycopy(buffer, from + length, tmp, from + dataLength, size - (from + length));
			System.arraycopy(data, dataOffset, tmp, from, dataLength);
			buffer = null;
			buffer = tmp;
		} else {
			System.arraycopy(buffer, from + length, buffer, from + dataLength, size - (from + length));
	 ...
	 * @Package: [com.cp.utils.StreamUtils.java]
	 * @Description: [BigEndian模式读取整型数据]
	 * @Author: [jacarri]
	 * @CreateDate: [2014-1-20 上午10:41:58]
	 * @Version: [v1.0]
	 */
	...
	public static long readLong(InputStream in) throws IOException {
		...
		long result = (b0 << 56) | (b1 << 48) | (b2 << 40) | (b3 << 32) | (b4 << 24) | (b5 << 16) | (b6 << 8) | (b7);
		return result;
	}
	...
	public static String readChar(InputStream in, int length) throws IOException {
